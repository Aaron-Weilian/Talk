...
public class GetData extends DBUtil {
	 ...
	 * Ŀ¼: so
	 */
	private boolean getCPCK() {
		try {
			//search file list
			Vector vecFileList = getFileList("so");
		
			DataBaseObject dbo = getDataBaseObject();
			for (int i=0; i < vecFileList.size(); i++) {
				try {
					SAXReader saxReader = new SAXReader();
					Document document = saxReader.read(MAIN_DIR+"/so/"+(String)vecFileList.get(i));
					Element root = document.getRootElement();
					//���ڳ�Ʒ
					Iterator iter_root = root.elementIterator();
					
					CommonBean cbCPCKHeader = new CommonBean();
					cbCPCKHeader.setBeanName("ie_ck_table");
					cbCPCKHeader.setAttribute("insert");
					
					CommonBean cbCPCKItem = new CommonBean();
					cbCPCKItem.setBeanName("ie_ck");
					cbCPCKItem.setAttribute("insert");
					
					String strHeaderID = null;
					
					while (iter_root.hasNext()) {
						//���ڳ�Ʒ����Ϣ - ͷ��Ϣ
						strHeaderID = com.aiyun.common.util.Oid.getOid();
						cbCPCKHeader.addValue("id",strHeaderID);
						cbCPCKHeader.addValue("adddate","_SYSDATE");
						cbCPCKHeader.addValue("oper_flag","-1");
						
						Element element_entry = (Element)iter_root.next();
						
						Iterator iter_entry = element_entry.elementIterator();
						while (iter_entry.hasNext()) {
							Element element_so = (Element)iter_entry.next();
							
							String h_name = element_so.getName();
							String h_text = element_so.getText();
							if (h_name.equalsIgnoreCase("organization")) {
								if (h_text.equalsIgnoreCase("ac")) {
									h_text="VP";
								}
								cbCPCKHeader.addValue(h_name,h_text);
							}
							else if (!h_name.equalsIgnoreCase("item") && !h_name.equalsIgnoreCase("organization")) {
								cbCPCKHeader.addValue(h_name,h_text);
							}
							//���ڳ�Ʒ����Ϣ - ��ϸ��Ϣ
							else if (h_name.equalsIgnoreCase("item")) {
								Iterator iter_item = element_so.elementIterator();
								cbCPCKItem.addValue("id",com.aiyun.common.util.Oid.getOid());
								cbCPCKItem.addValue("objectid",strHeaderID);
								while (iter_item.hasNext()) {
									Element element_item = (Element)iter_item.next();
									String i_name = element_item.getName();
									String i_text = element_item.getText();
									if (i_name.equalsIgnoreCase("item_no")) cbCPCKItem.addValue("cpno",i_text);
									else if (i_name.equalsIgnoreCase("version")) cbCPCKItem.addValue("version",i_text);
									else if (i_name.equalsIgnoreCase("quantity")) {
									
										//cbCPCKItem.addValue("spint",StrTool.str2double(i_text)/1000+"");
										cbCPCKItem.addValue("spint",i_text);
									}
			
									//��λ
									else if (i_name.equalsIgnoreCase("sldw")) {
										cbCPCKItem.addValue(i_name.toLowerCase(),(String)hmUnit.get(i_text.toUpperCase()));
									}
									else if (i_name.equalsIgnoreCase("amount")) cbCPCKItem.addValue("zj",i_text);
									else if (i_name.equalsIgnoreCase("price")) cbCPCKItem.addValue("dj",StrTool.str2double(i_text)*1000+"");
									else if (i_name.equalsIgnoreCase("delivery_date")) cbCPCKItem.addValue("delivery_date",i_text);
									else if (i_name.equalsIgnoreCase("arrive_date")) cbCPCKItem.addValue("arrive_date",i_text);
									else if (i_name.equalsIgnoreCase("job_order")) cbCPCKItem.addValue("job_order",i_text);
								}
								cbCPCKItem.addValue("adddate","_SYSDATE");
							}
						}
							
						String flag = cbCPCKHeader.getCellStr(cbCPCKHeader.getRowNum()-1,"order_type");
						if (flag!=null) {
							switch (com.aiyun.common.tool.StrTool.str2int(flag)) {
								case 1:
									flag = "8";
									break;
								case 2:
							...
							cbCPCKHeader.setCellObj(cbCPCKHeader.getRowNum()-1,"order_type",flag);
						}
					}
					boolean bFlag = true;
					if (cbCPCKHeader.getRowNum()>0) {
						bFlag = dbo.execute(cbCPCKHeader) && dbo.execute(cbCPCKItem);
					}
					
					//ɾ����Ҫɾ����δ����ͱ��Ϊɾ���ĵ���ĳ���SO����
					if (bFlag && cbCPCKHeader.getRowNum()>0) {
						bFlag = bFlag && dbo.execute("INSERT INTO CL_CUSTOMER (SELECT distinct CUS_NO,CUS_NAME FROM IE_CK_TABLE WHERE CUS_NO NOT IN (SELECT ID FROM CL_CUSTOMER) AND CUS_NO IS NOT NULL AND OPER_FLAG=-1)");
						//����:��λ��Ϣ��������,���unit=054��Ҫ��1000,���򲻳�1000
						bFlag = bFlag && dbo.execute("UPDATE IE_CK SET spint=spint/1000 where SLDW='054' And objectid in (select id from ie_ck_table where oper_flag=-1)");
						bFlag = bFlag && dbo.execute("UPDATE IE_CK_TABLE SET CURR=(SELECT ID FROM CL_CURR WHERE SENAME=IE_CK_TABLE.CURR) WHERE OPER_FLAG=-1");
						bFlag = bFlag && dbo.execute("UPDATE IE_CK_TABLE SET COUNTRY=(SELECT ID FROM CL_COUNTRY WHERE SENAME=IE_CK_TABLE.COUNTRY) WHERE OPER_FLAG=-1");
						bFlag = bFlag && dbo.execute("UPDATE IE_CK_TABLE SET po_no=substr(po_no,3) WHERE order_type=8 and OPER_FLAG=-1");
						
						bFlag = bFlag && dbo.execute("UPDATE IE_CK_TABLE SET OPER_FLAG=0 WHERE OPER_FLAG=-1");
					}
					
					//ɾ���ļ�
					if (bFlag) {
						File f = new File(MAIN_DIR+"/so/"+(String)vecFileList.get(i));
