...
public class RequestToEventTranslator implements Serializable {
            ...
            throws SystemException
    {
        ...
        ScreenFlowManager screenflowmanager = (ScreenFlowManager) servletcontext
                .getAttribute(ScreenFlowManager.KEY_NAME);
        URLMapping urlmapping = screenflowmanager.getURLMapping((String) httpservletrequest.getAttribute("pathInfo"));
        if (urlmapping == null) {
            Log.info(this, String.valueOf(String.valueOf((new StringBuffer("no url mapping for PathInfo(")).append(s).append("). use default PathInfo(/default)"))));
            urlmapping = screenflowmanager.getURLMapping("/default");
        }
        Log.info(this, "pathInfo is ".concat(String.valueOf(String.valueOf(s))));
        if (urlmapping == null) {
            throw new CommonException(String.valueOf(String.valueOf((new StringBuffer("no url mapping entry for [")).append(s).append("] in requestmappings.xml"))));
        }
        ...
        if (urlmapping.usePreProcessHandler()) {
            ...
            try {
                ...
                Log.error(this, String.valueOf(String.valueOf((new StringBuffer("fail to load PreProcessHandler[")).append(s1).append("]"))));
        ...
        if (urlmapping.useRequestHandler()) {
            ...
            Log.info(this, String.valueOf(String.valueOf((new StringBuffer("Event Class Name(path info is:")).append(s).append("):").append(s2))));
            if (s2 == null || s2.length() <= 0) {
                Log.info(this, String.valueOf(String.valueOf((new StringBuffer("Event class is not created.(Path-Info:")).append(s).append(")"))));
            }
            ...
            try {
                ...
                Log.error(this, String.valueOf(String.valueOf((new StringBuffer("fail to load RequestHandler[")).append(s3).append("]"))));
